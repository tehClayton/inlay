...
| `view` | How the neck is drawn, as settings named for what you see: `flip` (the bass edge, a guitar's low E, at the top instead of the bottom), `squeeze` (string spacing closing up away from the bass edge), `recession` (the headstock end drawn smaller), `angle` (the neck turned on screen, headstock end up) and `edge` (the fretboard's side along the bass edge, with side dots). Presets are starting points: **Tab** (the default: low string at the bottom, flat), **Flipped** (low string on top) and **Player's view** (tab with the strings squeezed as seen looking down across them, and the edge showing). The editor adjusts from any preset with sliders and a live preview. The bass edge is named by physical side rather than pitch, because on re-entrant tunings the face-side string isn't the lowest |
